...
const ProfileUpdate = () => {
  ...
  const { id } = useParams();
  const profile = useSelector(selectProfileDetail);
  ...
  useEffect(() => {
    try {
      ...
      setCurrentImage(profile.avatar_url || "");
      setSelectedResumeName(""); // reset selected new file name
      setPreview(null);
    } catch (error) {
      console.log(error)
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      Swal.fire({
        title: "Failed",
        text: "Your Profile is updated failed!",
        icon: "error",
        timer: 1500,
      });
      console.log(error);
  ...
  const resumeLink = profile?.resume_url ? `${import.meta.env.VITE_BASE_URL}/storage/${profile?.resume_url}` : null;
  const displayedImage = preview ||
    (currentImage
            ...
            <div className="space-y-2 flex flex-col">
              ...
              {profile?.resume_url ? (
                <div className="flex items-center gap-3">
                  <p>Old File: {profile?.resume_url}</p>
                  <a
                    href={resumeLink}
                    target="_blank"
                    rel="noreferrer"
